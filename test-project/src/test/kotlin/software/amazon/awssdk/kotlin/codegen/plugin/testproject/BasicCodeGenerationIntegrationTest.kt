...
class BasicCodeGenerationIntegrationTest {
    ...
    fun canUseStreamingOperations() {
        ...
        val key = "object.txt"
        s3.putObject(PutObjectRequest(bucket = bucketName, key = key), RequestBody.fromFile(testFile))

        val contents = s3.getObject(GetObjectRequest(bucket = bucketName, key = key), ResponseTransformer<GetObjectResponse, String> { _, inputStream ->
