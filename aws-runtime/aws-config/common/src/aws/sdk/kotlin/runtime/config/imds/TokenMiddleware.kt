...
internal class TokenMiddleware(config: Config) : Feature {
    ...
    override fun <I, O> install(operation: SdkHttpOperation<I, O>) {
        operation.execution.mutate.intercept { req, next ->
            val clock = req.context.getOrNull(SdkClientOption.Clock) ?: Clock.System
            val token = useCachedTokenOrClear(clock) ?: getToken(clock, req).also { cachedToken = it }
            req.subject.headers.append(X_AWS_EC2_METADATA_TOKEN, token.value.decodeToString())
            next.call(req)
        }
    }

    // use the cached token if it's still valid or clear it if expired and return null
    private fun useCachedTokenOrClear(clock: Clock): Token? {
        val cached = cachedToken ?: return null
        val now = clock.now()
        val expired = now.epochSeconds >= cached.expires.epochSeconds - TOKEN_REFRESH_BUFFER_SECONDS
        if (expired) {
            cachedToken = null
        }
        return cachedToken
        ...
        return try {
            when (call.response.status) {
                HttpStatusCode.OK -> {
                    ...
                    val expires = Instant.fromEpochSeconds(clock.now().epochSeconds + ttl, 0)
                    Token(token, expires)
                }
