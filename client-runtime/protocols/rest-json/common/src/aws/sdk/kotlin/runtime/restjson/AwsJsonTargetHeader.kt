...
public class AwsJsonTargetHeader(config: Config) : Feature {
    ...
    override fun <I, O> install(operation: SdkHttpOperation<I, O>) {
        operation.execution.state.intercept { req, next ->
            ...
            req.request.headers.append("X-Amz-Target", "$serviceName.$operationName")
            ...
            //  is complete.
            if (!req.request.headers.contains("Content-Type")) {
                req.request.headers.append("Content-Type", "application/x-amz-json-$version")
            }
